...
if methode == "Doe een voorstel":
    ...
    df_solver = pd.read_pickle("data/veggie_recipes_struct.pkl")
    recipe = st.selectbox("Kies een recept:", df_solver["name"])
    st.write("Gekozen recept:", recipe)
    st.dataframe(df_solver, width="content")


    drempel_eiwit = st.slider("Stel eiwit-drempel in (gram):", min_value=1.0, max_value=100.0, value=10.0, step=0.1)
    drempel_kcal = st.slider("Stel max kcal in:", min_value=1.0, max_value=3000.0, value=1000.0, step=1.0)
    min_serv_vse   = st.number_input("Minimale portiegrootte (VSE)", value=0.1, min_value=0.1, step=0.1)
    ...
    if st.button("🔎 Maak voorstel"):
        for i_wd,wd in enumerate(["ma","di","wo","do","vr","za","zo"]):
            ...
            if plan_df is None:
                ...
                df_solver = df_solver[~df_solver['name'].isin(plan_df['Recept'])]
